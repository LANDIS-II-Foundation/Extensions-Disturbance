    ...
    public class ManagementArea
    {
        ...
        public void HarvestStands()
        {
            ...
            foreach (AppliedPrescription prescription in prescriptions) {
                ...
                {
                    //UI.WriteLine("Applying Prescription: {0}  Model.Core.CurrentTime: {1}", prescription.Prescription.Name, Model.Core.CurrentTime);
                    
                    if (isDebugEnabled)
                        log.DebugFormat("  Initializing prescription {0} ...", prescription.Prescription.Name);
                        
                    //UI.WriteLine("  Initializing prescription {0} ...", prescription.Prescription.Name);
            ...
            foreach (AppliedPrescription prescription in prescriptions) {
                //UI.WriteLine("      Looping through prescriptions... {0}.", prescription.Prescription.Name);
            
                if (prescription is AppliedRepeatHarvest) 
                {
                    prescription.Prescription.SiteSelectionMethod = new CompleteStand();
                    //UI.WriteLine("      Attempting to Re-Harvest {0}.", prescription.Prescription.Name);
