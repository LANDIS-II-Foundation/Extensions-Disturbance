...
namespace Landis.Extension.DroughtGenerator
{
...

    {
        ...
        public override void Run()
        {
            ...
            // Output carried in SiteVars represents # years drought per 100 decades
            // Output to map represents # years drought per 10 decades
            ...
            using (IOutputRaster<BytePixel> outputRaster = modelCore.CreateRaster<BytePixel>(path, modelCore.Landscape.Dimensions))
            {
                ...
                foreach (Site site in modelCore.Landscape.AllSites)
                {
                    if (site.IsActive)
                    {
                        pixel.MapCode.Value = (byte)Math.Round((double)SiteVars.DroughtYears[site] / 10.0);
                    }
                    else
                    {
                        //  Inactive site
