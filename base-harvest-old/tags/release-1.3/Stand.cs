...
namespace Landis.Harvest
{
        ...
        : IEnumerable<ActiveSite>
    {
        ...
        public ActiveSite GetRandomActiveSite {
            get
            {
                //List<ActiveSite> sites = this.GetSites();
                ...
                int random = (int)(Landis.Util.Random.GenerateUniform() * (siteLocations.Count - 1));
                if(random < 0 || random > siteLocations.Count - 1)
                    return null;
                //    throw new System.ApplicationException("Stand contains no active cells");
        ...
        public ushort ComputeAge()
        {
            double total = 0.0;
            foreach (ActiveSite site in this) {
                total += (double) AgeCohort.Util.GetMaxAge(Model.LandscapeCohorts[site]);
            }
